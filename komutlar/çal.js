...
module.exports = {
  name: "çal",
  cooldown: 3,
  aliases: ["p","oynat","play"],
  description: "YouTubeden müzik çalarsınız.",
  async execute(message, args) {
    ...
    if (!args.length)
      return message
        .reply(`Usage: ${message.client.prefix}çal <YouTube URL | Müzik İsmi>`)
        .catch(console.error);
    ...
    if (urlValid) {
      ...
      try {
        ...
        if (error.statusCode === 404)
          return message.reply("SoundCloud kullanılamıyor.").catch(console.error);
        return message.reply("SoundCloud kullanılamıyor.").catch(console.error);
      }
    } else {
      try {
        ...
        console.error(error);
        return message.reply("Aramanıza uygun video bulunamadı.").catch(console.error);
      }
    }
    ...
    try {
      ...
      await channel.leave();
      return message.channel.send(`Kanala katılamıyor: ${error}`).catch(console.error);
    }
  }
};
